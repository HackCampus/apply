...
const nonStatusEventTypes = values(applicationEvents).filter(({ affectsStatus }) => !affectsStatus).map(({ type }) => type);
...
module.exports = bsModels => {
  ...
  return class ApplicationEvent {
    ...
    static async fetchAll(where) {
      ...
      const bs = await BsModel.where(where).orderBy('ts', 'DESC').fetchAll({ withRelated: 'actor' });
