...
module.exports = bsModels => {
  ...
  return class User {
    ...
    async updateAuthentication(authentication, transaction) {
      return definitelyTransact(transaction, async transaction => {
        const existingAuthentication = await new bsModels.Authentication({ userId: this.id, type: authentication.type }).fetch({ transacting: transaction });
        if (existingAuthentication) {
          authentication.updatedAt = new Date();
          try {
            const authenticationModel = await existingAuthentication.save(authentication, { patch: true, transacting: transaction });
