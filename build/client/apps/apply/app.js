...
module.exports = Component({
      ...
      case 'saveApplication':
        {
          ...
          if (personalDetailsResponses.contactEmail && personalDetailsResponses.contactEmail.length === 0) {
  ...
  run(effect, sources, action) {
    const get = (url, handler) => pull(api.get(url), pull.map(handler));
    const put = (url, body, handler) => pull(api.put(url, body), pull.map(handler));
    switch (effect.type) {
      case 'waitForUser':
        {
          return pull(sources.models(), pull.map(model => model.children.authenticate.user), pull.filter(user => user != null), pull.take(1), pull.map(user => action('authenticated', user)));
        }
      case 'createApplication':
        {
          return put('/me/application', {}, ({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('createApplicationSuccess', data);
              default:
                return action('createApplicationError', data);
            }
          });
        }
      case 'fetchApplication':
        {
          return get('/me/application', ({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('fetchApplicationSuccess', data);
              case 'Not Found':
                return action('fetchApplicationNotFound', data);
              default:
                return action('fetchApplicationError', data);
            }
          });
        }
      case 'saveApplication':
        {
          ...
          return put('/me/application', application, ({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('saveApplicationSuccess', data);
              case 'Bad Request':
                return action('saveApplicationUserError', data);
              default:
                return action('saveApplicationError', data);
            }
          });
        }
      case 'saveTechPreferences':
        {
          ...
          return put('/me/application/techpreferences', techPreferences, ({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('saveTechPreferencesSuccess', data);
              default:
                return action('saveTechPreferencesError', data);
            }
          });
        }
      case 'autosave':
        {
          return pull(sources.actions(), pull.filter(action => ['personalDetails', 'techPreferences', 'questions'].indexOf(action.child) !== -1), uninterrupted(3000), pull.map(() => action('saveApplication')));
        }
      case 'connect':
        {
  ...
  view(model, dispatch, children) {
    ...
    `;
    const finishApplication = () => dispatch(action('saveApplication', { finished: true }));
    const connect = provider => dispatch(action('connect', provider));
    ...
    const props = readOnly ? { application, user, errorFields, readOnly } : { application, user, errorFields };
    return html`
      <div class="apply">
...
function getFormResponses(model) {
  ...
  for (let field in model.children) {
    const { value, started } = model.children[field];
    if (started) {
      // only send through the ones that have actually been updated
      fields[field] = value;
    }
  }
  return fields;
...
function deadlineMessage() {
  return pastDeadline ? html`<p><strong>The application deadline for the ${constants.programmeYear} programme was on the ${deadline}. You can still apply, but we can not guarantee that you will be considered for this year's interviews.</strong></p>` : html`<p>The application deadline for the ${constants.programmeYear} programme is the ${deadline}.</p>`;
}