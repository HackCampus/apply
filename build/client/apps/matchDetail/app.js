...
module.exports = Component({
  ...
  actionsView(model, dispatch, children) {
    ...
    return html`
      <div class="actions">
        ...
        <div class="events">
          ${events && events.length > 0 ? events.map(event => eventView(event, user, () => dispatch(action('deleteApplicationEvent', event.id)))) : html`<div class="event"><em>No events yet!</em></div>`}
        </div>
      </div>
    `;
  },
  run(effect, sources, action) {
    ...
    const post = (url, body, handler) => pull(api.post(url, body), pull.map(handler));
    const doDelete = (url, handler) => pull(api.delete(url), pull.map(handler));
    switch (effect.type) {
      case 'fetchApplication':
        {
          return get(`/applications/${applicationId}`, ({ statusText, data }) => {
            switch (statusText) {
              ...
              default:
                return action('fetchApplicationFailure', data);
            }
          });
        }
      ...
      case 'fetchApplicationEvents':
        {
          return get(`/applications/${applicationId}/events`, ({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('fetchApplicationEventsSuccess', data);
              default:
                return action('fetchApplicationEventsFailure', data);
            }
          });
        }
      ...
      case 'deleteApplicationEvent':
        {
          ...
          return doDelete(`/applications/${applicationId}/events/${eventId}`, ({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('deleteApplicationEventSuccess', data);
              default:
                return action('deleteApplicationEventFailure', data);
            }
          });
        }
      ...
      case 'fetchUser':
        {
          return get(`/me`, ({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('fetchUserSuccess', data);
              default:
                return action('fetchUserFailure', data);
            }
          });
        }
      ...
      case 'submit':
        {
          ...
          return post(`/applications/${applicationId}/events`, event, ({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('submitSuccess', data);
              default:
                return action('submitFailure', data);
            }
          });
        }
