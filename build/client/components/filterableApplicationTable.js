...
module.exports = Component({
  ...
  view(model, dispatch, children) {
    return h('div', { class: 'applications' }, [model.showFilters ? link('Hide filters', () => dispatch(action('hideFilters'))) : link('Show filters', () => dispatch(action('showFilters'))), model.showFilters ? this.filtersView(model, dispatch, children) : '', model.showFilters ? h('div', { class: 'applyFilters' }, [link('Apply filters', () => dispatch(action('fetchApplications')))]) : '', children.table ? children.table() : '']);
  },
  filtersView(model, dispatch, children) {
    return h('div', { class: 'filters' }, [filter('Application stages', children.stages()), filter('Tech preferences', children.techs()), filter('Genders', children.genders())]);

    function filter(name, child) {
      return h('div', { class: 'filterContainer' }, [h('h4', {}, [name]), child]);
    }
  },
