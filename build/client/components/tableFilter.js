...
module.exports = options => Component({
      ...
      case 'addFilter':
        {
          ...
          const active = model.active.indexOf(newValue) === -1 ? model.active.concat([newValue]) : model.active.slice();
          const children = {
  ...
  view(model, dispatch, children) {
    return h('div', { class: 'filter' }, [model.active.map((item, i) => h('div', { class: 'filterItem' }, [item, ' ', link('-', () => dispatch(action('removeFilter', i))), ' '])), children.select(), ' ', link('+', () => dispatch(action('addFilter')))]);
  },
  getActive(model) {
