...
module.exports = Component({
      ...
      case 'register':
        {
          ...
          return pull(api.post('/users', { email, password }), pull.map(({ statusText, data }) => {
            switch (statusText) {
              case 'Created':
                return action('registerSuccess', data);
              default:
                return action('registerError', data);
            }
          }));
        }
      case 'login':
      ...
      case 'changePassword':
        {
          ...
          return pull(api.put('/me/password', { password }), pull.map(({ statusText, data }) => {
            switch (statusText) {
              case 'OK':
                return action('changePasswordSuccess', data);
              default:
                return action('changePasswordError', data);
            }
          }));
        }
    }
  ...
  view(model, dispatch, children) {
    return model.user ? this.authenticatedView(model, dispatch, children) : this.notAuthenticatedView(model, dispatch, children);
  },
  authenticatedView(model, dispatch, children) {
    ...
    return html`
      <div class="authenticated">
        ...
        <div class="margin">
          ${listField('Actions', [html`<a href="/signout">Sign out</a>`, link('Change password', () => dispatch(action('showChangePasswordFields')))])}
        </div>
  ...
  notAuthenticatedView(model, dispatch, children) {
    return html`
      <div>
        ${listField('Authentication method', [link('GitHub', () => dispatch(action('oauth', 'github'))), link('LinkedIn', () => dispatch(action('oauth', 'linkedin'))), link('email/password', () => dispatch(action('showPasswordFields')))])}
        ${model.password ? this.passwordView(model, dispatch, children) : ''}
      </div>
    `;
  },
  passwordView(model, dispatch, children) {
    const select = id => dispatch(action('select', id));
    const radio = (id, content) => link(content, () => select(id), { class: model.tab === id ? 'disabled' : '' });
    return html`
      <div class="margin">
        ${listField('Actions', [radio(tabs.newApplication, 'Register - start a new application'), radio(tabs.existingApplication, 'Log in - edit an existing application')])}
        ${(() => {
      switch (model.tab) {
  ...
  newApplicationView(model, dispatch, children) {
    ...
    const valid = emailField.started && passwordField.started && confirmPasswordField.started && emailField.valid && passwordField.valid && confirmPasswordField.valid && passwordField.value === confirmPasswordField.value;
